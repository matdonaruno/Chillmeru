...
- **daily-jobsの429対策**（2026-08-29）: 8月後半、`daily-jobs`が断続的に失敗していた
  ...
  (2)全滅（新着>0なのに成功0）したときは、429などの待てば直る失敗なら
  データを書き換えず緑のまま次回に持ち越し、モデルID無効・キー無効のような
  人が直す必要のある失敗を一度でも見たときだけexit 1（`callLLM`が投げる
  エラーに`.transient`を載せて区別している）、(3)リトライ待ちを
