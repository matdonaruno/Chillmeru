...
// 使うモデル。リトライのたびに順繰りに切り替える。429 code 1305（混雑）は
// モデル単位で起きるため、別モデルに振れば通ることがある。
// 実測（2026-08-29 08:15 UTC、check-llmワークフロー）: 同じ時刻に
// glm-4.7-flash は5回連続で429、glm-4.5-flash は1回目で成功した。
// なおドキュメントに無料と載っている glm-4-flash-250414 は、このAPIキーでは
// `400 code 1211 模型不存在` になり使えなかったので候補に入れていない
// （候補を足すときは必ず `gh workflow run check-llm.yml -f model=<id>` で
//  実際に通ることを確かめること。当て推量のモデルIDは静かな失敗経路になる）。
const DEFAULT_GLM_MODELS = ["glm-4.7-flash", "glm-4.5-flash"];
...
export async function callLLM(prompt) {
  ...
  // この呼び出しの中で「使えない」と分かったモデルは候補から外していく。
  // 将来フォールバック先が廃止されて 400 code 1211 を返すようになっても、
  // 残ったモデルで処理を続けられるようにするため（フォールバックの故障が
  // 本命モデルまで道連れにしないこと）。
  const models = [...GLM_MODELS];
  let lastError = null;
  // 一度でも「待っても直らない」エラー（モデルIDが無効、キーが無効など）を見たか。
  // 投げるエラーに .transient として載せ、呼び出し側が「明日やり直せばいい失敗」と
  // 「人が直さないと直らない失敗」を区別できるようにする。
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    let hint = "";
    if (r.status === 400) hint = "\nヒント: モデルIDが無効か、リクエスト形式の問題。";
    else if (r.status === 401 || r.status === 403) hint = "\nヒント: APIキーが無効、または権限不足。";
    else if (r.status === 429) hint = "\nヒント: レート制限/無料枠超過、またはそのモデルの混雑。";
    if (!RETRYABLE_STATUSES.has(r.status)) sawPermanent = true;
    lastError = new Error(`llm ${r.status} (model=${model})${hint}\n${detail.slice(0, 300)}`);
    lastError.status = r.status;
    lastError.transient = !sawPermanent;
    ...
    // 待っても変わらないエラー。モデル固有の可能性があるので、他に候補が
    // 残っていればそのモデルを外して即座に切り替える（待ち時間は消費しない）。
    if (models.length > 1) {
      console.warn(`  ⚠ llm ${r.status}（model=${model} は使えないので候補から外す）`);
