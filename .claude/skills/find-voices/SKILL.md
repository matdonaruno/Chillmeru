...
**投稿文（6-2）の事実源は、確認済みの`summary_ja`だけ**。英語原文を読み直して情報を足さない。
vault側の素材メモ（6-1）は、収集中に読んだコメントまで含めてよい（ネタの判断材料にするため）が、
その場合も「投稿文に使うときは要約の範囲に収める」ことをメモ内に書いておく。
🔴 **Redditのスコアを人数に言い換えない**（318は投票の合計であって「318人の技師が賛同」ではない）。
反応の数を書くときは「大きな反応」「コメント◯件」のように、元の指標のまま書く。
